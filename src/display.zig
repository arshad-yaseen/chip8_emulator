...
pub const Display = struct {
    ...
    pub inline fn getPixel(self: *Display, x: u8, y: u8) u1 {
    ...
    pub inline fn isPixelOn(self: *Display, x: u8, y: u8) bool {
        return self.getPixel(x, y) != 0;
    }

    // toggle pixel
    pub inline fn togglePixel(self: *Display, x: u8, y: u8) void {
        const index = pixelIndex(x, y);
